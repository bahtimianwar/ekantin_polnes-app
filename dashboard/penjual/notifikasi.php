...
<div class="page-header">
    <h1><i class="fas fa-bell" style="color:#3949ab; margin-right:10px;"></i>Notifikasi</h1>
...
<style>
    ...
    .notif-count strong { color: #3949ab; }
    .btn-read-all {
        padding: 8px 18px;
    ...
    .btn-read-all:hover { background: #c5cae9; }
    ...
    .notif-item.unread { border-left: 4px solid #3949ab; }
    ...
    .icon-pembayaran { background: #e8f5e9; color: #2e7d32; }
    .icon-sistem { background: #f3e5f5; color: #6a1b9a; }
    .icon-promo { background: #fff3e0; color: #e65100; }
    .notif-body { flex: 1; }
    ...
    .notif-actions { display: flex; align-items: center; gap: 8px; margin-left: auto; flex-shrink: 0; }
    .dot-unread {
        width: 9px; height: 9px; border-radius: 50%;
        background: #3949ab; flex-shrink: 0;
        margin-top: 4px;
    }
    .btn-hapus {
...
<div class="notif-list">
    <?php foreach ($notifs as $n):
        $icon_class = 'icon-sistem';
        $icon_name = 'fa-info-circle';
        if (stripos($n['judul'], 'pesanan') !== false) { $icon_class = 'icon-pesanan'; $icon_name = 'fa-shopping-bag'; }
        elseif (stripos($n['judul'], 'bayar') !== false || stripos($n['judul'], 'saldo') !== false) { $icon_class = 'icon-pembayaran'; $icon_name = 'fa-wallet'; }
        elseif (stripos($n['judul'], 'promo') !== false) { $icon_class = 'icon-promo'; $icon_name = 'fa-tag'; }
        
        $waktu = new DateTime($n['tanggal']);
        $now = new DateTime();
        $diff = $now->diff($waktu);
        if ($diff->days > 0) $time_str = $diff->days . ' hari lalu';
        elseif ($diff->h > 0) $time_str = $diff->h . ' jam lalu';
        else $time_str = $diff->i . ' menit lalu';
    ?>
    <div class="notif-item <?= $n['is_read'] ? 'read' : 'unread' ?>">
        ...
        <div class="notif-actions">
            <?php if (!$n['is_read']): ?>
                <div class="dot-unread" title="Belum dibaca"></div>
            <?php endif; ?>
            <a href="?hapus=<?= $n['id_notif'] ?>" class="btn-hapus" onclick="return confirm('Hapus notifikasi ini?')" title="Hapus">
